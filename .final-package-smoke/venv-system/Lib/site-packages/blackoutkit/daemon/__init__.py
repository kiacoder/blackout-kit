...
def run_daemon_loop(engine_name: str, env_overrides_json: str | None = None):
    ...
    try:
        ...
        if s.get("auto_set_proxy", True):
            if cleanup_owned_system_proxy():
                log.info("Blackout-managed system proxy restored.")
            else:
                log.info("System proxy ownership was absent or changed; leaving current proxy untouched.")
        log.info("Done.")
